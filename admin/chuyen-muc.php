...
?>
                    <tr>
                      ...
                      <td ><?=$row['pin'];?></td>
                      <td ><?=mysqli_fetch_assoc($ketnoi->query("SELECT COUNT(*) FROM `taikhoan` WHERE `code` = '".$row['code']."' AND `username` IS NULL  AND status = 'live' ")) ['COUNT(*)']; ?> CLONE LIVE</td>
                      <td ><?=mysqli_fetch_assoc($ketnoi->query("SELECT COUNT(*) FROM `taikhoan` WHERE `code` = '".$row['code']."' AND `username` IS NOT NULL  ")) ['COUNT(*)']; ?> CLONE</td>
                      <td >
                        <?php
                          if ($row['display'] == 'hide')
                          {
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              <form role="form" action="" method="post">
                  ...
                  <div class="form-group">
                    <label for="exampleInputEmail1">LOẠI TÀI KHOẢN (Muốn check live thì để: facebook hoặc clone)</label>
                    <input type="text" class="form-control" name="pin" placeholder="Hệ thống tự động check live khi loại = facebook hoặc clone, via" >
                  </div>
                  <div class="form-group">
