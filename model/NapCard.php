...
if(isset($_POST['btnNapCard']) && isset($_SESSION['username']))
{
    ...
    {
        $json = json_decode(curl_get('https://card24h.com/api/card-auto.php?auto=true&type='.$loaithe.'&menhgia='.$menhgia.'&seri='.$seri.'&pin='.$pin.'&APIKey='.$site['site_api_card'].'&callback='.$site_callback.'&content='.$code), true);
        if (isset($json['data']))
        {
